...
- If the initial user request included “提交/推送”, then after the user confirms the report content, write and submit directly without asking an extra confirmation about Git actions. If it only asked to fill/update, write only.

## Workflow

1. Run `git switch develop` and `git pull --rebase origin develop` in the Tiangong repo once before reading/writing, to align with the latest `develop`.
